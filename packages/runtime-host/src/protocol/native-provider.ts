...
export const NATIVE_PROVIDER_CAPABILITIES = ['computer_use'] as const;
...
export interface NativeProviderSubcallFrame extends NativeProviderSubcallIdentity {
...
export interface NativeProviderSessionReleaseIdentity {
  readonly hostEpoch: string;
  readonly registrationId: string;
...
export interface NativeProviderSessionReleaseFrame extends NativeProviderSessionReleaseIdentity {
  readonly kind: 'native.provider.session_release';
}

export interface NativeProviderSessionReleasedFrame extends NativeProviderSessionReleaseIdentity {
  readonly kind: 'native.provider.session_released';
}
...
export type NativeProviderResultOutcome =
  | { readonly ok: true; readonly result: NativeProviderResultPayload }
  | {
      readonly ok: false;
      readonly error: { readonly code: NativeProviderFailureCode };
    };

export type NativeProviderResultFrame = NativeProviderSubcallIdentity &
  Readonly<{ kind: 'native.provider.result' }> &
  NativeProviderResultOutcome;
...
export function isNativeProviderClientFrameKind(
  ...
  return (
    ...
    value === 'native.provider.session_released'
  );
}
...
export function isNativeProviderHostFrameKind(
  ...
  return (
    ...
    value === 'native.provider.session_release'
  );
}
...
export function decodeNativeProviderClientFrame(value: unknown): NativeProviderClientFrame {
  ...
  if (record.kind === 'native.provider.session_released') {
    return decodeNativeProviderSessionReleasedFrame(record);
  }
...
export function decodeNativeProviderHostFrame(value: unknown): NativeProviderHostFrame {
  ...
  if (record.kind === 'native.provider.session_release') {
    return decodeNativeProviderSessionReleaseFrame(record);
  }
...
export function decodeNativeProviderSubcallFrame(value: unknown): NativeProviderSubcallFrame {
  ...
  if (frame.kind !== 'native.provider.subcall' || frame.capability !== 'computer_use') {
  ...
  return {
    ...
    subcall: boundedInlinePayload(
      decodeNativeProviderSubcall(frame.subcall),
...
export function decodeNativeProviderReleaseFrame(value: unknown): NativeProviderReleaseFrame {
  ...
  return {
    ...
    ...decodeInvocationIdentity(frame),
  };
}

export function decodeNativeProviderSessionReleaseFrame(
  value: unknown,
): NativeProviderSessionReleaseFrame {
  const frame = requireExactRecord(value, 'native Provider session release frame', [
    'kind',
    'hostEpoch',
    'registrationId',
    'releaseId',
  ...
  if (frame.kind !== 'native.provider.session_release') {
    throw invalidProtocolFrame('Invalid Native Provider session release');
  }
  return {
    kind: 'native.provider.session_release',
    ...decodeSessionReleaseIdentity(frame),
  };
}

export function decodeNativeProviderSessionReleasedFrame(
  value: unknown,
): NativeProviderSessionReleasedFrame {
  const frame = requireExactRecord(value, 'native Provider session released frame', [
    'kind',
    'hostEpoch',
    'registrationId',
    'releaseId',
  ...
  if (frame.kind !== 'native.provider.session_released') {
    throw invalidProtocolFrame('Invalid Native Provider session released acknowledgement');
  }
  return {
...
export function decodeNativeProviderResultFrame(value: unknown): NativeProviderResultFrame {
  ...
  const result = boundedInlinePayload(
    decodeNativeProviderResultPayload(success.result),
    'native Provider result payload',
  );
  if (
    nativeProviderResultAttachmentRefs(result).length > NATIVE_PROVIDER_MAX_ATTACHMENTS_PER_RESULT
  ) {
    throw invalidProtocolFrame('Native Provider result has too many attachments');
  }
  return { kind: 'native.provider.result', ...identity, ok: true, result };
}

export function decodeNativeProviderSubcall(value: unknown): NativeProviderSubcall {
...
export function decodeNativeProviderResultPayload(value: unknown): NativeProviderResultPayload {
...
function decodeSessionReleaseIdentity(
  record: Record<string, unknown>,
): NativeProviderSessionReleaseIdentity {
  return {
    hostEpoch: requireId(record.hostEpoch, 'hostEpoch'),
...
function decodeSubcallIdentity(record: Record<string, unknown>): NativeProviderSubcallIdentity {
  return {
    ...
    ordinal: positiveBoundedInteger(
      record.ordinal,
      'native Provider subcall ordinal',
...
function boundedInlinePayload<T>(value: T, label: string): T {
  ...
  if (
    ...
    throw invalidProtocolFrame(`${label} exceeds the byte limit`);
  }
  return value;
}
