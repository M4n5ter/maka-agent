...
test('disconnect while operation admission is pending cannot attach phantom continuity', async () => {
  ...
  try {
    ...
    await withTimeout(firstRun, 1_000, 'connection session did not settle after admission release');
    ...
    const response = decodeHostFrame(await secondPair.clientTransport.read(1_000));
...
function createConnectionHandlers(continuity: SessionContinuityCoordinator): OperationHandlerMap {
...
async function openTransportPair(directory: string, name: string): Promise<TransportPair> {
